...
def test_training_plan_migration_creates_revision_and_membership_tables(tmp_path: Path) -> None:
    ...
    assert {"user_id", "week_start", "current_revision_id", "accepted_revision_id"} <= {
...
def test_weekly_plan_accepted_revision_migration_preserves_plans_and_cycle_links(
    tmp_path: Path,
) -> None:
    database_path = tmp_path / "weekly-plan-accepted-revision.db"
    database_url = f"sqlite:///{database_path.as_posix()}"
    config = Config("alembic.ini")
    config.attributes["database_url"] = database_url
    command.upgrade(config, "20260828_33")
    engine = create_engine(database_url)
    ...
    with engine.begin() as connection:
        ...
        connection.exec_driver_sql(
            "INSERT INTO training_plans "
            "(id, user_id, week_start, status, current_revision_id, created_at, updated_at) VALUES "
            "(1, 1, '2026-08-31', 'active', NULL, '2026-08-28', '2026-08-28'), "
            "(2, 1, '2026-09-07', 'active', NULL, '2026-08-28', '2026-08-28'), "
            "(3, 2, '2026-08-31', 'active', NULL, '2026-08-28', '2026-08-28')"
        )
        for revision_id, plan_id, owner_id, fingerprint in (
            (1, 1, 1, "plan-1-revision-1"),
            (2, 1, 1, "plan-1-revision-2"),
            (3, 2, 1, "plan-2-revision-1"),
            (4, 3, 2, "plan-3-revision-1"),
        ):
            connection.exec_driver_sql(
                "INSERT INTO training_plan_revisions "
                "(id, plan_id, owner_user_id, source_assistant_message_id, revision_number, "
                "week_start, week_end, planner_version, knowledge_base_version, "
                "input_fingerprint, generation_context_json, validation_report_json, "
                "created_at) VALUES (?, ?, ?, NULL, ?, '2026-08-31', '2026-09-06', "
                "'weekly-v1', 'kb-v1', ?, '{}', '{\"valid\": true}', '2026-08-28')",
                (revision_id, plan_id, owner_id, 2 if revision_id == 2 else 1, fingerprint),
            )
        connection.exec_driver_sql("UPDATE training_plans SET current_revision_id = 2 WHERE id = 1")
        connection.exec_driver_sql(
            "INSERT INTO training_cycles "
            "(id, user_id, goal_id, event_type, start_date, target_date, status, "
            "current_revision_id, accepted_revision_id, created_at, updated_at) VALUES "
        ...
        connection.exec_driver_sql(
            ...
            "knowledge_base_version, input_fingerprint, confidence, phase_plan_json, "
            "assumptions_json, impact_json, validation_report_json, created_at) VALUES "
            "(1, 1, 1, NULL, NULL, 1, '10k', '2026-08-31', '2026-10-25', "
            "'cycle-v1', 'kb-v1', 'cycle-1', 'medium', '[]', '{}', '{}', "
            "'{\"valid\": true}', '2026-08-28')"
        )
        connection.exec_driver_sql(
            "INSERT INTO training_cycle_weeks "
    ...
    accepted_column = next(
        ...
        if column["name"] == "accepted_revision_id"
    )
    assert accepted_column["nullable"] is True

    with engine.begin() as connection:
        assert connection.exec_driver_sql(
            "SELECT current_revision_id, accepted_revision_id FROM training_plans WHERE id = 1"
        ).one() == (2, None)
        assert connection.exec_driver_sql(
            "SELECT cycle_revision_id, training_plan_revision_id FROM training_cycle_weeks"
        ).one() == (1, 1)
        connection.exec_driver_sql(
            "UPDATE training_plans SET accepted_revision_id = 1 WHERE id = 1"
        )
        connection.exec_driver_sql("UPDATE training_plans SET current_revision_id = 1 WHERE id = 1")
        assert connection.exec_driver_sql(
            "SELECT current_revision_id, accepted_revision_id FROM training_plans WHERE id = 1"
    ...
    for revision_id in (3, 4):
        with (
            pytest.raises(IntegrityError, match="belong to its plan and user"),
            engine.begin() as connection,
        ):
            connection.exec_driver_sql(
                "UPDATE training_plans SET accepted_revision_id = ? WHERE id = 1",
                (revision_id,),
            )
    with (
        pytest.raises(IntegrityError, match="belong to its plan and user"),
        ...
        connection.exec_driver_sql(
            "INSERT INTO training_plans "
            "(id, user_id, week_start, status, current_revision_id, "
            "accepted_revision_id, created_at, updated_at) VALUES "
            "(5, 1, '2026-09-14', 'active', NULL, 1, '2026-08-28', '2026-08-28')"
        )
    engine.dispose()


def test_weekly_plan_accepted_revision_migration_rejects_invalid_current_pointer(
    tmp_path: Path,
) -> None:
    database_path = tmp_path / "weekly-plan-invalid-current.db"
    database_url = f"sqlite:///{database_path.as_posix()}"
    config = Config("alembic.ini")
    config.attributes["database_url"] = database_url
    command.upgrade(config, "20260828_33")
    ...
    with engine.begin() as connection:
        ...
        connection.exec_driver_sql(
            ...
            "week_start, week_end, planner_version, knowledge_base_version, input_fingerprint, "
            "generation_context_json, validation_report_json, created_at) VALUES "
            "(1, 1, 1, NULL, 1, '2026-08-31', '2026-09-06', 'weekly-v1', 'kb-v1', "
            "'plan-1', '{}', '{\"valid\": true}', '2026-08-28')"
        )
        connection.exec_driver_sql(
            "INSERT INTO training_plans "
            "(id, user_id, week_start, status, current_revision_id, created_at, updated_at) VALUES "
            "(2, 1, '2026-09-07', 'active', 1, '2026-08-28', '2026-08-28')"
        )
    engine.dispose()

    with pytest.raises(RuntimeError, match="weekly plan revision pointer conflict"):
        command.upgrade(config, "head")
...
def test_garmin_fit_authorization_upgrade_preserves_operations_and_constraints(
    tmp_path: Path,
) -> None:
    database_path = tmp_path / "garmin-fit-authorization.db"
    database_url = f"sqlite:///{database_path.as_posix()}"
    config = Config("alembic.ini")
    config.attributes["database_url"] = database_url
    command.upgrade(config, "20260829_36")
    engine = create_engine(database_url)
    ...
    with engine.begin() as connection:
        ...
        connection.exec_driver_sql(
            "INSERT INTO users (id, display_name, created_at) VALUES "
            "(1, 'Runner', '2026-09-01'), (2, 'Other', '2026-09-01')"
        )
        connection.exec_driver_sql(
            "INSERT INTO workouts "
            "(id, user_id, name, sport, status, definition_version, definition, source_type, "
            "approval_status, local_schedule_status, lock_version, created_at, updated_at) VALUES "
            "(1, 1, 'Run', 'running', 'confirmed', 1, '{\"blocks\": []}', 'manual', "
            "'accepted', 'unscheduled', 0, '2026-09-01', '2026-09-01'), "
            "(2, 2, 'Other Run', 'running', 'confirmed', 1, '{\"blocks\": []}', 'manual', "
            "'accepted', 'unscheduled', 0, '2026-09-01', '2026-09-01')"
        )
        connection.exec_driver_sql(
            "INSERT INTO workout_revisions "
            "(id, workout_id, revision_number, name, sport, definition_version, definition, "
            "source_type, content_hash, edit_source, created_at) VALUES "
            "(1, 1, 1, 'Run', 'running', 1, '{\"blocks\": []}', 'manual', ?, 'manual', "
            "'2026-09-01'), "
            "(3, 1, 2, 'Run v2', 'running', 1, '{\"blocks\": []}', 'manual', ?, 'manual', "
            "'2026-09-01'), "
            "(2, 2, 1, 'Other Run', 'running', 1, '{\"blocks\": []}', 'manual', ?, 'manual', "
            "'2026-09-01')",
            ("a" * 64, "c" * 64, "b" * 64),
        )
        connection.exec_driver_sql(
            "UPDATE workouts SET current_revision_id = id, accepted_revision_id = id, "
            "materialized_revision_id = id, accepted_at = '2026-09-01'"
        )
        connection.exec_driver_sql(
            "INSERT INTO workout_garmin_bindings "
            "(id, workout_id, content_status, calendar_status, device_status) VALUES "
            "(1, 1, 'unknown', 'not_requested', 'not_requested'), "
            "(2, 2, 'not_requested', 'not_requested', 'not_requested')"
        )
        operation_id = connection.exec_driver_sql(
            "INSERT INTO workout_garmin_operations "
            "(workout_id, binding_id, operation_type, revision_id, remote_identity_id, "
            "idempotency_key, status, remote_reference, completed_at, error_code, created_at) "
            "VALUES (1, 1, 'upload', 1, NULL, ?, 'unknown', 'remote-1', '2026-09-01', "
            "'garmin.remote_outcome_unknown', '2026-09-01')",
            ("c" * 64,),
        ).lastrowid
        connection.exec_driver_sql(
            "INSERT INTO workout_garmin_attempts "
            "(operation_id, attempt_number, attempt_kind, status, started_at, completed_at, "
            "error_code, error_message) VALUES "
            "(?, 1, 'execute', 'unknown', '2026-09-01', '2026-09-01', "
            "'garmin.remote_outcome_unknown', 'response lost')",
            (operation_id,),
        )
    engine.dispose()
    ...
    columns = {
        column["name"]: column
        for column in inspect(engine).get_columns("workout_garmin_operations")
    }
    assert authorization_columns <= columns.keys()
    assert all(columns[name]["nullable"] for name in authorization_columns)

    with engine.connect() as connection:
        assert connection.exec_driver_sql(
            "SELECT operation_type, revision_id, idempotency_key, status, remote_reference, "
            "completed_at, error_code, training_fit_policy_version, "
            ...
            "training_fit_authorized_revision_id FROM workout_garmin_operations WHERE id = 1"
        ).one() == (
            "upload",
            1,
            "c" * 64,
        ...
        assert connection.exec_driver_sql(
            "SELECT attempt_number, attempt_kind, status, error_code, error_message "
            "FROM workout_garmin_attempts WHERE operation_id = 1"
        ).one() == (
            1,
            "execute",
            "unknown",
    ...
    with engine.begin() as connection:
        connection.exec_driver_sql("PRAGMA foreign_keys=ON")
        authorized_operation_id = connection.exec_driver_sql(
            "INSERT INTO workout_garmin_operations "
            "(workout_id, binding_id, operation_type, revision_id, remote_identity_id, "
            "idempotency_key, status, created_at, training_fit_policy_version, "
            "training_fit_assessment_fingerprint, training_fit_effective_date, "
            "training_fit_acknowledged_by_user_id, training_fit_acknowledged_at, "
            "training_fit_authorized_revision_id) VALUES "
            "(1, 1, 'upload', 1, NULL, ?, 'pending', '2026-09-01', ?, ?, ?, ?, ?, ?)",
            ("e" * 64, *authorization_values),
        ).lastrowid

    with pytest.raises(IntegrityError), engine.begin() as connection:
        connection.exec_driver_sql(
            "UPDATE workout_garmin_operations SET revision_id = 3 WHERE id = ?",
    ...
    with pytest.raises(IntegrityError), engine.begin() as connection:
        ...
        connection.exec_driver_sql(
            "INSERT INTO workout_garmin_operations "
            "(workout_id, binding_id, operation_type, revision_id, remote_identity_id, "
            "idempotency_key, status, created_at, training_fit_policy_version) VALUES "
            "(1, 1, 'upload', 1, NULL, ?, 'pending', '2026-09-01', 'training-fit-v1')",
            ("f" * 64,),
        )
    ...
    with pytest.raises(IntegrityError), engine.begin() as connection:
        ...
        connection.exec_driver_sql(
            ...
            "training_fit_acknowledged_by_user_id, training_fit_acknowledged_at, "
            "training_fit_authorized_revision_id) VALUES "
            "(1, 1, 'upload', 1, NULL, ?, 'pending', '2026-09-01', ?, ?, ?, 2, ?, ?)",
            ("g" * 64, *authorization_values[:3], *authorization_values[4:]),
        )
    ...
    with pytest.raises(IntegrityError), engine.begin() as connection:
        connection.exec_driver_sql(
            ...
            "(workout_id, binding_id, operation_type, revision_id, remote_identity_id, "
            "idempotency_key, status, created_at) VALUES "
            "(1, 1, 'upload', 1, NULL, ?, 'invalid', '2026-09-01')",
            ("i" * 64,),
        )
    engine.dispose()
...
def test_drop_coach_tool_calls_preserves_coach_state_and_recreates_schema_on_downgrade(
    ...
    config = Config("alembic.ini")
    config.attributes["database_url"] = database_url
    command.upgrade(config, "20260906_38")
    engine = create_engine(database_url)
    ...
    with engine.begin() as connection:
        ...
        connection.exec_driver_sql(
            ...
            "prompt_template_version, operation_contract_version, failure_category, "
            "artifacts_json, created_at, completed_at) VALUES "
            "(1, 1, 'user', 'Plane meinen Lauf', 'completed', NULL, NULL, NULL, NULL, NULL, "
            "'[]', '2026-09-06 08:00:00', '2026-09-06 08:00:00'), "
            "(2, 1, 'assistant', 'Hier ist dein Plan.', 'completed', 'model-a', 'request-1', "
            "'coach-prompt-v2', 'coach-operations-v1', NULL, "
            '\'[{"type": "workout", "id": 7}]\', '
        ...
        connection.exec_driver_sql(
            ...
            "(id, conversation_id, user_message_id, assistant_message_id, workout_id, status, "
            "model_id, request_id, created_at, completed_at) VALUES "
            "(1, 1, 1, 2, NULL, 'completed', 'model-a', 'request-1', "
            "'2026-09-06 08:00:01', '2026-09-06 08:00:02')"
        )
        connection.exec_driver_sql(
            "INSERT INTO coach_tool_calls "
            "(id, message_id, call_id, tool_name, label, input_summary, status, started_at, "
            "completed_at, error_message) VALUES "
            "(1, 2, 'completed-call', 'get_health_day', 'Gesundheit geprüft', NULL, "
            "'completed', '2026-09-06 08:00:01', '2026-09-06 08:00:02', NULL), "
            "(2, 2, 'unresolved-call', 'get_progress', 'Fortschritt geprüft', NULL, "
            "'running', '2026-09-06 08:00:02', NULL, NULL)"
        )
    engine.dispose()
    ...
    assert "coach_tool_calls" not in inspector.get_table_names()
    with engine.connect() as connection:
        assert connection.exec_driver_sql(
            "SELECT id, user_id, title FROM coach_conversations"
        ).one() == (1, 1, "Training")
        assert connection.exec_driver_sql(
            "SELECT id, role, content, status, artifacts_json FROM coach_messages ORDER BY id"
        ).all() == [
            (1, "user", "Plane meinen Lauf", "completed", "[]"),
            (
                2,
                "assistant",
                "Hier ist dein Plan.",
        ...
        assert connection.exec_driver_sql(
            "SELECT conversation_id, user_message_id, assistant_message_id, status, request_id "
            "FROM coach_assistant_runs"
        ).one() == (1, 1, 2, "completed", "request-1")
        assert connection.exec_driver_sql("PRAGMA foreign_key_check").all() == []
        assert (
            connection.exec_driver_sql("SELECT version_num FROM alembic_version").scalar_one()
            == "20260906_39"
