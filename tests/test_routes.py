...
def test_main_pages_render(client: TestClient) -> None:
    ...
    assert "Dein Laufcoach" in client.get("/coach").text
...
def test_edit_draft_workout(client: TestClient, session_factory: sessionmaker[Session]) -> None:
    ...
    assert "/static/css/tailwind.css?v=20260913-1" in form.text
...
def _seed_planning_history(session_factory: sessionmaker[Session]) -> date:
    with session_factory() as session:
        user = session.scalar(select(User))
        assert user is not None
        today = date.today()
