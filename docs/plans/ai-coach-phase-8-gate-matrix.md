...
| Template expansion | Only active `easy_run` is selected; duration is the requested time capped at 90 minutes (`min(90, available_minutes)`, `easy-run-candidate-v2`), bounded by 20-90 minutes | `test_easy_run_proposal_is_deterministic_revisioned_and_unscheduled`, `test_easy_run_proposal_uses_requested_60_minutes` |
